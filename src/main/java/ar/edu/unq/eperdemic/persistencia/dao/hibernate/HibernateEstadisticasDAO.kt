...
class HibernateEstadisticasDAO : EstadisticasDAO {
    ...
    override fun especieQueInfectaAMasVectoresEn(nombreUbicacion: String): String {
        val q1 = "(SELECT DISTINCT e.nombre, COUNT(*) AS occ FROM Especie e INNER JOIN Vector_Especie ve ON e.id = ve.especies_id AND ve.Vector_id IN (SELECT v.id FROM Vector v WHERE v.ubicacion_nombreUbicacion=:nombreUbicacion ORDER BY e.nombre DESC) GROUP BY e.id ORDER BY occ DESC LIMIT 1) AS q;"
